...
func NewKVDistributedLock(id string, kvStore KVStore) *KVDistributedLock {
...
func (l KVDistributedLock) extractIndex(key string) (int, error) {
	...
	if len(splitKey) < 3 {
		...
		return -1, common.NewInternalError(msg)
	}
...
func (l KVDistributedLock) getMaxIndex(entries []string) (int, error) {
	...
	if len(entries) == 0 {
		return -1, common.NewInternalError("no entries to process")
	}
...
func (l KVDistributedLock) Lock(ctx context.Context, timeout time.Duration) (context.Context, error) {
	...
	for {
		...
		if err == nil {
			break
		} else if !errors.Is(err, &common.ConflictError{}) {
			return ctx, err
		}
		myIndex++
		myKey = l.getIndexKey(myIndex)
	...
	for myIndex != minIndex {
		...
		case <- timeoutChannel:
			t.Stop()
			return ctx, common.NewTimedOutError("lock timed out")
		}
		minIndex, err = l.getMinWaiter(ctx)
		if err != nil {
			return ctx, err
	...
	ctx = common.InjectDistributedLockIndex(ctx, myIndex)
	return ctx, nil
}
...
func (l KVDistributedLock) Unlock(ctx context.Context) error {
	...
	idx := common.ExtractDistributedLockIndex(ctx)
	if idx != minIndex {
		return common.NewInvalidError("cannot call unlock when you do not hold the lock")
	}
