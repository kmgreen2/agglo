...
func DoCompletion(numMaps, numJoined int, timeout time.Duration, missingJoinKey string, forceTimeout bool) (int, int,
	int, error) {
	...
	completer := core.NewCompleter(completion, kvStore)
...
func TestCompletionHappyPath(t *testing.T) {
	...
	for i := 0; i < numRuns; i++ {
		...
		numComplete, numTriggered, numTimedOut, err := DoCompletion(numMaps, numJoins, -1, "", false)
		if err != nil {
...
func TestCompletionTimeoutNotify(t *testing.T) {
	numRuns := 4
	maxMaps := 128
	...
	for i := 0; i < numRuns; i++ {
		...
		numComplete, numTriggered, numTimedOut, err := DoCompletion(numMaps, numJoins,
...
func TestCompletionIncomplete(t *testing.T) {
	numRuns := 64
	maxMaps := 128
